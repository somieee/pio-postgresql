...
typedef struct EState
{
	...
	//FIXME: DASOM
	ItemPointerData _tidList[100]; // __IndexNext
	int numtid;
	int numAlloctids; // for ___IndexNext
	ItemPointerData *_tidListptr;
	ItemPointer __tidList; //for ___IndexNext
//	long numberTuples; //for __IndexNext - meaningless any more.
	/*
	 * when deleting numberTuples, check where I used.
	 * -execUtils.c, ExecPlan func - maybe and etc
	 */
} EState;
...
typedef struct TidScanState
{
	...
	HeapTupleData tss_htup[MAX_LIBAIO]; //FIXME:DASOM

	/* FIXME:DASOM add new variables for AIO*/
	int numberOfmbuf;
	Buffer xs_mbuf[MAX_LIBAIO];
	int cur_tuple;
	bool multiple_read;
	int i; //for counting how many times heap_fetch_multiple is called
//	bool keep_buf[MAX_LIBAIO];
} TidScanState;
