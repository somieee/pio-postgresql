...
/* FIXME:DASOM */
//static TupleTableSlot *_IndexNext(IndexScanState *node);
//static TupleTableSlot *__IndexNext(IndexScanState *node);
//static TupleTableSlot *___IndexNext(IndexScanState *node);

static TupleTableSlot *P_IndexNext(IndexScanState *node);

/*static void _StoreIndexTuple(TupleTableSlot *slot, IndexTuple itup,
				TupleDesc itupdesc); // just copy and paste
*/
...
#if 0
static TupleTableSlot *
P_IndexNext(IndexScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
...
P_IndexNext(IndexScanState *node)
{
	...
	ItemPointer tid;
	ItemPointerData *_tidList;
	//int *numTids;
	long numberTuples = MAX_LIBAIO;
	BlockNumber blkNum;
	unsigned int relid;
	Relation rel;
	Relation heapRelation;
	static long numTid = 0;
	static long cur_pos = 0;
//	_tidList = (ItemPointerData *)palloc(MAX_LIBAIO*sizeof(ItemPointerData));
	...
	heapRelation = scandesc->heapRelation;
	buffer = scandesc->xs_mbuf;
	tuple = scandesc->xs_mtup;
	/*
	 * FIXME:DASOM
	*/ 
	if(estate->_tidListptr == NULL)
	{
		estate->_tidListptr = (ItemPointerData *) 
								palloc(MAX_LIBAIO * sizeof(ItemPointerData));
	}
	//numTids = &(estate->numtid);
	rel = scandesc->indexRelation;
	relid = rel->rd_id;
	
	/*
	   check if I have appropriate tidList or not
	*/
	for(;;)
	{
		if(!(node->filled))
		{
			while((tid = __index_getnext(scandesc, direction)) != NULL)
			{
				estate->_tidListptr[numTid]=*tid;		
				//fprintf(stderr, "%dth IN, blk: %u, offset: %u \n", numTid, ItemPointerGetBlockNumber(&estate->_tidListptr[numTid]), ItemPointerGetOffsetNumber(&estate->_tidListptr[numTid]));
				numTid++;
			
				if(numTid == MAX_LIBAIO)
				{
					node->filled=true;
					numTid=0;
					break;
		...
		if(node->filled) 
		{
			if(!(node->multi_read))
			{
				if(heap_fetch_multiple(heapRelation, snapshot, tuple, false, NULL, buffer, MAX_LIBAIO, estate->_tidListptr))
					node->multi_read=true;
			}
				
			if(node->multi_read)
			{	
				//fprintf(stderr, "good\n");
				while(cur_pos < MAX_LIBAIO)
				{
					ExecStoreTuple(&tuple[cur_pos],	/* tuple to store */
								   slot,	/* slot to store in */
								   scandesc->xs_mbuf[cur_pos],		/* buffer containing tuple */
								   false);	/* don't pfree */
					/*
					* If the index was lossy, we have to recheck the index quals using
					* the fetched tuple.
					*/
					...
					ReleaseBuffer(scandesc->xs_mbuf[cur_pos]);
					cur_pos++;
					return slot;
				}
			}
		}
		if(cur_pos == MAX_LIBAIO)
		{
			node->multi_read=false;
			node->filled=false;
			cur_pos=0;
		}
	}
	...
	if(node->multi_read)
	{
		if(i < MAX_LIBAIO)
		{
			ExecStoreTuple(&tuple[i],	/* tuple to store */
						   slot,	/* slot to store in */
						   scandesc->xs_mbuf[i],		/* buffer containing tuple */
						   false);	/* don't pfree */
			/*
	 		* If the index was lossy, we have to recheck the index quals using
 			* the fetched tuple.
			...
			ReleaseBuffer(scandesc->xs_mbuf[i]);
			i++;
			return slot;
		}
		else
...
ExecIndexScan(IndexScanState *node)
{
	...
	return ExecScan(&node->ss,
					(ExecScanAccessMtd) P_IndexNext,//IndexNext,   /*FIXME:DASOM IndexNext->__IndexNext*/
...
ExecInitIndexScan(IndexScan *node, EState *estate, int eflags)
{
	...
	indexstate->ss.ps.ps_TupFromTlist = false;//I guess I put this.
