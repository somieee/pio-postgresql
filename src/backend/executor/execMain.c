					 ...
					 ScanDirection direction, long count)
{
	 ...
	 */
#if 0
	elog(LOG,"dasom : standard_executor dest : %d",dest->mydest);
#endif
			...
			DestReceiver *dest)
{
	...
	BlockNumber blk;
	OffsetNumber off;
	EState *_estate; // allocation of an array for storting tids
	/* DASOM : dynamic allocation! */
	_estate = estate;
	//_estate->_tidListptr = (ItemPointerData *)palloc(numberTuples * sizeof(ItemPointerData)); 
//	_estate->numberTuples = numberTuples;
...
//	 _estate->numberTuples= numberTuples;
	...
	for (;;)
	{
		...
		if (sendTuples){
			 ...
			 * DASOM
			 */
			//elog(LOG,"som dest : %d",dest->mydest);	
