...
#if 0
	if (BufferIsValid(scan->xs_mbuf[0]))
	{
		for(i=0;i<MAX_LIBAIO;i++)
		{
			ReleaseBuffer(scan->xs_mbuf[i]);
			scan->xs_mbuf[i] = InvalidBuffer;
		}
	}
#endif
...
index_getnext(IndexScanDesc scan, ScanDirection direction)
{
	...
	for (;;)
	{
		if (scan->xs_continue_hot)
		{
			...
			//DASOM
			//blkNum = ItemPointerGetBlockNumber(&scan->xs_ctup.t_self);
		}
		else
		{
		...
		relid = scan->indexRelation->rd_id;
#if 0
		elog(LOG,"SOM index_getnext: %u",relid);
#endif
 ...
 */
ItemPointer
__index_getnext(IndexScanDesc scan, ScanDirection direction)
{
	//HeapTuple	heapTuple; //FIXME:DASOM
	ItemPointer tid;
 ...
 */
HeapTuple
_index_getnext(IndexScanDesc scan, ScanDirection direction)
{
	HeapTuple	heapTuple;
	ItemPointer tid;
	/*
	 * Add a few variables by SOM
	 */
	static unsigned int som2=0; /*DASOM*/
	BlockNumber blk_som;
	OffsetNumber off_som;
	...
	for (;;)
	{
		...
		//	blk_som = ItemPointerGetBlockNumber(&scan->xs_ctup.t_self);
		//	off_som = ItemPointerGetOffsetNumber(&scan->xs_ctup.t_self);
		//	if(blk_som == 16392 || blk_som == 16389) {
		//		elog(LOG, "%u dasom if : %u, %u",som2, blk_som, off_som);
		//		som2++;
		//	}
		}
		else
		{
			...
			/*
			 * DASOM
			 * check if tid is equal to c_tid in HeapTupleHeaderData or not
		
			blk_som = ItemPointerGetBlockNumber(&scan->xs_ctup.t_self);
			off_som = ItemPointerGetOffsetNumber(&scan->xs_ctup.t_self);

			if(blk_som ==16385 || blk_som == 16389) {
				elog(LOG, "%u dasom else : %u, %u", som2, blk_som, off_som);
				som2++;
			}*/
...
/*FIXME:DASOM*/
/*ItemPointer index_getnextTid(IndexScanDesc scan, ScanDirection direction)
{
	ItemPointer tid;
