...
/* GUC variable */
bool		synchronize_seqscans = true; //what is it for?
...
/* 
 * FIXME:DASOM 
 * This is for multiple IO in TidNext()
 */

bool
heap_fetch_multiple(Relation relation,
		   Snapshot snapshot,
		   HeapTuple tuple, //caller sends HeapTupleData array
		   bool keep_buf, // fixed value; FALSE
		   Relation stats_relation,
		   Buffer *m_buffer,
		   int mbufNum,
		   ItemPointerData *tidList)
{
//	ItemPointer tid = &(tuple->t_self); //array?
	ItemId		lp[mbufNum];
	Page		page[mbufNum]; // array
	OffsetNumber offnum;
	bool		valid[mbufNum];
	 ...
	 */
	//readbuffermultiple
	ReadBufferMultiple(relation, m_buffer, mbufNum, tidList);

	/*
	 * Need share lock on buffer to examine tuple commit status.
	 */
	//multiple lock // buffer -> m_buffer
	for(i = 0; i < mbufNum; i++)
	{
		LockBuffer(m_buffer[i], BUFFER_LOCK_SHARE);
		page[i] = BufferGetPage(m_buffer[i]);

	   /*
	 	* We'd better check for out-of-range offnum in case of VACUUM since the
	 	* TID was obtained.
	 	*/
		offnum = ItemPointerGetOffsetNumber(&tidList[i]);
		if (offnum < FirstOffsetNumber || offnum > PageGetMaxOffsetNumber(page[i]))
		{
			LockBuffer(m_buffer[i], BUFFER_LOCK_UNLOCK);
			if(!keep_buf)
			{
				ReleaseBuffer(m_buffer[i]);
				elog(LOG,"out-of-range release"); /*DASOM log*/
			//	*userbuf = InvalidBuffer;
			}
			tuple[i].t_data = NULL;
			return false;
		 ...
		 * get the item line pointer corresponding to the requested tid
		 */
		lp[i] = PageGetItemId(page[i], offnum);

		/*
		 * Must check for deleted tuple.
		 */
		if (!ItemIdIsNormal(lp[i]))
		{
			LockBuffer(m_buffer[i], BUFFER_LOCK_UNLOCK);
			if (!keep_buf)
			{
				ReleaseBuffer(m_buffer[i]);
				elog(LOG,"abnormal tuple released"); /*DASOM log*/
			//	*userbuf = InvalidBuffer;
			}
			tuple[i].t_data = NULL;
			return false;
		 ...
		 * fill in *tuple fields
		 */
		tuple[i].t_data = (HeapTupleHeader) PageGetItem(page[i], lp[i]);
		tuple[i].t_len = ItemIdGetLength(lp[i]);
		tuple[i].t_tableOid = RelationGetRelid(relation);

		/*
		 * check time qualification of tuple, then release lock
		 */
		valid[i] = HeapTupleSatisfiesVisibility(&tuple[i], snapshot, m_buffer[i]);
		
//		fprintf(stderr,"%dth tuple : buffer-%d\n",i,m_buffer[i]); /*DASOM log*/
		
		if (valid[i])
			PredicateLockTuple(relation, &tuple[i], snapshot);
		else
			test_valid=false;

		CheckForSerializableConflictOut(valid[i], relation, &tuple[i], m_buffer[i], snapshot);
	...
	if(!keep_buf){
		ReleaseBuffer(m_buffer[i]);
		elog(LOG,"end of heap-fetch-multiple release"); /*DASOM log*/
	}
