 ...
 * FIXME:DASOM
 */
#include "storage/dasom_bufmgr.h"
 ...
 *	FIXME:DASOM
 */
void ReadBufferMultiple(Relation reln, Buffer *mbuf, int mbufNum, ItemPointerData *tidList)
{
	ReadBufferExtendedMultiple(reln, MAIN_FORKNUM, mbuf, mbufNum, tidList, RBM_NORMAL, NULL);
}
				   ...
				   ReadBufferMode mode, BufferAccessStrategy strategy)
{
	...
	/*
	 * COMMENT:DASOM
	 * relpersistence is about types of relation; regular table, unlogged permanent table or temporary table
	 */
	...
	/*
	 * DASOM
	 * print out information of read request
	 * FOR SSD_TABLES
	 *
	if((reln->rd_id==16820)||(reln->rd_id==16817)) {
		elog(LOG, "%u dasom : %c, %u, %u, %c",som1,reln->rd_rel->relkind,reln->rd_id,blockNum,(hit)?'H':'M');
	}*/
	/*
	 * For HDD_TABLES
	if ((reln->rd_id==16701)||(reln->rd_id==16704)) {
		elog((LOG, "%u dasom : %c, %u, %u, %c",som1,reln->rd_rel->relkind,reln->rd_id,blockNum,(hit)?'H':'M');
	}
	 */
//	som1++;
 ...
 */
void
ReadBufferExtendedMultiple(Relation reln, ForkNumber forkNum, Buffer *mbuf, 
					int mbufNum, ItemPointerData *tidList,
				   	ReadBufferMode mode, BufferAccessStrategy strategy)
{
	bool		hit[mbufNum];
//	Buffer		buf; //dasom
	 ...
	 */
	pgstat_count_buffer_read(reln);
	ReadBufferMultiple_common(reln->rd_smgr, reln->rd_rel->relpersistence, forkNum, tidList,
							mbufNum, mbuf, mode, strategy, hit);
				  ...
				  BufferAccessStrategy strategy, bool *hit)
{
	...
	*hit = false;
	...
	if (isExtend)
	{
#if 0 /* woonhak_buf_detector*/
		fprintf(stderr, "current block is extended buf\n"); 
		fprintf(stderr, "current block is local buf : %s\n", isLocalBuf ? "local" : "global"); 
...
#if 0
		fprintf(stderr, "rel extend done\n"); 
#endif
	}
	else
	{
#if 0
		fprintf(stderr, "Nope current block is global buf\n"); 
		fprintf(stderr, "current block is local buf : %s\n", isLocalBuf ? "local" : "global"); 
#endif
		/*
...
/*
 * status : done
 * FIXME:DASOM 21st Nov 2014
 * *tidList is for keeping multiple blocknumbers 
 */
void
ReadBufferMultiple_common(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
				  ItemPointerData *tidList, int mbufNum, Buffer *mbuf, ReadBufferMode mode,
				  BufferAccessStrategy strategy, bool *hit)
{
	//a single variable -> array
	volatile BufferDesc *bufHdr[mbufNum];
	Block		bufBlock;
	bool		found[mbufNum];
	bool		isExtend[mbufNum];
	//adding new variables
	bool		check_extend;
	int 		i;
	int			miss_count;
	int cb_list_index; //for smgr_read_insert.
	BlockNumber blkNum[mbufNum];
	...
	{
		hit[i] = false;
		blkNum[i] = ItemPointerGetBlockNumber(&tidList[i]);
	}
	...
	for(i = 0; i < mbufNum ; i++)
	{
		...
		isExtend[i] = (blkNum[i] == P_NEW);

		TRACE_POSTGRESQL_BUFFER_READ_START(forkNum, blkNum[i],
										   smgr->smgr_rnode.node.spcNode,
										   smgr->smgr_rnode.node.dbNode,
										   smgr->smgr_rnode.node.relNode,
										   smgr->smgr_rnode.backend,
		...
		{
			check_extend = true;
			blkNum[i] = smgrnblocks(smgr, forkNum);
		}

		if (isLocalBuf)
		{
			bufHdr[i] = LocalBufferAlloc(smgr, forkNum, blkNum[i], &found[i]);
			mbuf[i] = BufferDescriptorGetBuffer(bufHdr[i]); //newly added
		...
		else
		{
		 	...
		 	* not currently in memory.
		 	*/
			bufHdr[i] = BufferAlloc(smgr, relpersistence, forkNum, blkNum[i],
								 strategy, &found[i]);
			if (found[i])
				pgBufferUsage.shared_blks_hit++;
			else
	...
	// allocation aiocb ; newly added
	if(miss_count > 0 && !check_extend)
		smgr_read_start(smgr, miss_count);

	/* At this point we do NOT hold any locks. */
	//warning : not close FOR loop.
	for(i = 0; i < mbufNum; i++)
	{
		/* if it was already in the buffer pool, we're done */
		if (found[i])
		{
			if (!isExtend[i])
			{
				...
				TRACE_POSTGRESQL_BUFFER_READ_DONE(forkNum, blkNum[i],
												  smgr->smgr_rnode.node.spcNode,
												  smgr->smgr_rnode.node.dbNode,
												  smgr->smgr_rnode.node.relNode,
												  smgr->smgr_rnode.backend,
				...
				mbuf[i] = BufferDescriptorGetBuffer(bufHdr[i]);
				continue;
			}
		 	...
		 	* always have left a zero-filled buffer, complain if not PageIsNew.
		 	*/
			bufBlock = isLocalBuf ? LocalBufHdrGetBlock(bufHdr[i]) : BufHdrGetBlock(bufHdr[i]);
			if (!PageIsNew((Page) bufBlock))
				ereport(ERROR,
				 		(errmsg("unexpected data beyond EOF in block %u of relation %s",
										 blkNum[i], relpath(smgr->smgr_rnode, forkNum)),
			...
			{
				/* Only need to adjust flags */
				Assert(bufHdr[i]->flags & BM_VALID);
				bufHdr[i]->flags &= ~BM_VALID;
			}
			else
			{
				/*
				...
				do
				{
					...
					Assert(bufHdr[i]->flags & BM_VALID);
					bufHdr[i]->flags &= ~BM_VALID;
					UnlockBufHdr(bufHdr[i]);
				} while (!StartBufferIO(bufHdr[i], true));
			}
		} //the end of if(found)
		 ...
		 * again.
		 */
		Assert(!(bufHdr[i]->flags & BM_VALID));		/* spinlock not needed */
		...
		if (isExtend[i])
		{
			...
			MemSet((char *) bufBlock, 0, BLCKSZ);
			/* don't set checksum for all-zero page */
			smgrextend(smgr, forkNum, blkNum[i], (char *) bufBlock, false);

			/* Set BM_VALID, terminate IO, and wake up any waiters. */
			TerminateBufferIO(bufHdr[i], false, BM_VALID);
		}
		else
		{
			if (mode == RBM_ZERO)
			...
			{
				smgr_read_insert(smgr, forkNum, blkNum[i], (char *) bufBlock,
						cb_list_index);
				cb_list_index++;
			}
		}
	...
	if(!check_extend && miss_count > 0)
	{
		...
		smgr_aio_read(smgr, miss_count);
		smgr_read_end(smgr, miss_count);
	...
	for( i = 0; i < mbufNum ; i++)
	{
		if(!found[i] && !isExtend[i])
		{
			if(mode != RBM_ZERO)
			{
				bufBlock = isLocalBuf ? LocalBufHdrGetBlock(bufHdr[i]) : BufHdrGetBlock(bufHdr[i]);

				/* check for garbage cost */	
				if (!PageIsVerified((Page) bufBlock, blkNum[i]))
				{
					if (mode == RBM_ZERO_ON_ERROR || zero_damaged_pages)
					{
						ereport(WARNING,
 ...
 *
Buffer 
ReleaseAndReadBufferMultiple(Buffer c_buffer, Buffer *m_buffer, 
					Relation relation, int *tidList, BlockNumber blockNum, 
					int mbufNum, int prev_mbufNum, int currPos, bool *check)
{
	ForkNumber	forkNum = MAIN_FORKNUM;
	volatile BufferDesc *bufHdr;
...
PinBuffer(volatile BufferDesc *buf, BufferAccessStrategy strategy)
{
	...
	//fprintf(stderr, "PinBuffer buf : %03d, refcnt :%u , PrivataRefcnt : %d\n",b+1, buf->refcount, PrivateRefCount[b]);
	return result;
}
...
PinBuffer_Locked(volatile BufferDesc *buf)
{
	...
	ResourceOwnerRememberBuffer(CurrentResourceOwner,
								BufferDescriptorGetBuffer(buf));
//	fprintf(stderr, "PinBuffer_Locked buf : %03d, refcnt :%u , PrivataRefcnt : %d\n",b+1, buf->refcount, PrivateRefCount[b]);
}
...
UnpinBuffer(volatile BufferDesc *buf, bool fixOwner)
{
	...
	//fprintf(stderr, "UnpinBuffer buf : %03d, refcnt :%u , PrivataRefcnt : %d\n",b+1, buf->refcount, PrivateRefCount[b]);
...
ReleaseBuffer(Buffer buffer)
{
	...
	//fprintf(stderr, "ReleaseBuffer buf : %03d, refcnt :%u , PrivataRefcnt : %d\n",buffer, bufHdr->refcount, PrivateRefCount[buffer-1]);
}
...
IncrBufferRefCount(Buffer buffer)
{
	...
	if (BufferIsLocal(buffer))
		...
		PrivateRefCount[buffer - 1]++;
	//fprintf(stderr, "IncrRefCount buf : %03d , PrivataRefcnt : %d\n",buffer, PrivateRefCount[buffer -1]);
