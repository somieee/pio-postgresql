...
static int io_wait_run(io_context_t ctx, long iter);
/* delete it
   static int io_wait_remains(io_context_t ctx, long iter);
*/
/* FIXME : DASOM */
#if 1
/* Back trace wrapper
      void btrace(func_name, lineno)
func_name   : function name of caller
lineno      : line no from the caller
 
 */
#define BTRACE_DEPTH 20
void btrace(const char *func_name, int lineno){

	void *trace_array[BTRACE_DEPTH];
	size_t trace_size;

	/* get void*'s for all entries on the stack */
	trace_size = backtrace(trace_array, BTRACE_DEPTH);

	/*FIXME:fix this to use elog */
	fprintf(stderr, "%s:%d-back trace result : \n", func_name, lineno);
	backtrace_symbols_fd( trace_array, trace_size, STDERR_FILENO);
...
#ifdef DIRECT_IO
	fd = PathNameOpenFile(path, O_RDWR | O_CREAT | O_EXCL | PG_BINARY |O_DIRECT, 0600);
#else
...
#ifdef DIRECT_IO
			fd = PathNameOpenFile(path, O_RDWR | PG_BINARY | O_DIRECT, 0600);
#else			
...
/*FIXME:DASOM*/
#ifdef DIRECT_IO
		fd = OpenTransientFile(path, O_RDWR | PG_BINARY | O_DIRECT, 0);
#else
		 ...
		 char *buffer, bool skipFsync)
{
	...
	fprintf(stderr, "bad write buffer addr : %p, align:%d\n", buffer, (unsigned int) buffer%512);
	btrace(__func__, __LINE__);
#endif
		if (nbytes < 0)
			ereport(ERROR,
					(errcode_for_file_access(),
...
/* FIXME : DASOM */
void 
md_read_start(int miss_count)
{
	int rc;

	ioq = (struct iocb **) malloc(sizeof(struct iocb *) * miss_count);

	memset(&myctx, 0, sizeof(io_context_t));

	rc = io_queue_init(miss_count, &myctx);
	if(rc != 0)
	{
		elog(LOG, "io_queue_init error!");
	}
}

void
md_read_insert(SMgrRelation reln, ForkNumber forkNum,
		BlockNumber blocknum, char *buffer, int index)
{
	off_t seekpos;
	MdfdVec *v;
	...
	io_prep_pread(io, GetFileFd(v->mdfd_vfd), buffer, BLCKSZ, seekpos);
...
rd_done(io_context_t *ctx, struct iocb *iocb, long res, long res2)
{
	...
	{
		fprintf(stderr, "read missing byte expect %d got %d0", iocb->u.c.nbytes, res);
		exit(1);
	}
...
void
md_aio_read(int miss_count)
{
	int rc, res;

	rc = io_submit(myctx, miss_count, ioq);
	if(rc < 0)
	{
		elog(LOG, "io_submit fail - %s\n", sys_errlist[-rc]);
		exit(1);
	}

	wait_count = rc;
#if 0
	res = io_wait_run(myctx, rc);
	fprintf(stderr, "first requests: %d\n", res);
#endif

	while(wait_count)
	{
#if 1
		//res = io_wait_run(myctx, wait_count);
		res = io_wait_run(myctx, wait_count);
		fprintf(stderr,"miss_count : %d, io_submit : %d, io_get_events : %d, wait_count : %d\n", miss_count, rc, res, wait_count);
	...
	/* delete it
		if (res < rc)
		{
			//io_wait_remains(myctx,wait_count)
		}
	*/
		if(res < 0)
		{
			elog(LOG, "io_wait_run error!");
			exit(1);
		}
	}
}
...
static int
io_wait_run(io_context_t ctx, long iter)
{
	...
	int i=0;
#if 1
	struct timespec *timeout = (struct timespec*) malloc(sizeof(struct timespec));

	timeout->tv_sec = 0;
	timeout->tv_nsec = 5000000000;
#endif

#if 0
	ret = n = io_getevents(ctx, iter, iter, events, NULL);
#else
	ret = io_getevents(ctx, iter, iter, events, timeout);
#endif
	//DASOM: add io_getevents error handling
	while ((ret < 0))
	{
		if (ret == EINTR)
		{
			fprintf(stderr, "YEP, Interrupted!\n");
		}
		fprintf(stderr, "%dth io_getevents error : res %ld, ret %d err %s\n", i, events[iter].res, ret, strerror(-ret));
		ret = io_getevents(ctx, iter, iter, events, timeout);
		i++;
		//exit(1);
	}
...
void
md_read_end(int miss_count)
{
	int i = 0;
	int rc;

	rc = io_queue_release(myctx);
	if(rc !=0)
		elog(LOG, "io_queue_release error!");
			  ...
			  int oflags)
{
	...
	/* open the file */
#ifdef DIRECT_IO
	fd = PathNameOpenFile(fullpath, O_RDWR | PG_BINARY | oflags | O_DIRECT, 0600);
#else	
...
#ifdef DIRECT_IO
					char	   *tmp, *zerobuf;
					tmp = palloc0(BLCKSZ*2);
					zerobuf = SECTORALIGN(tmp);
